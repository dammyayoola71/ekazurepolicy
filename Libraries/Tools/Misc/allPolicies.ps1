    ...
    foreach ($item in $content) {
        ...
        if ($item.Attributes -eq "Directory") {
            ...
            $content1 = Get-ChildItem ($policyDefRootFolder + "\" + $($dir.Name) + "\" + $($item.Name))
                ...
                if ($content1.Attributes -eq "Directory") {
                    Write-Host "    ##[debug] checking directory"

                    $content2 = Get-ChildItem ($policyDefRootFolder + "\" + $($dir.Name) + "\" + $($item.Name))

                    foreach ($content in $content2) {
                        $policies += ($policyDefRootFolder + "\" + $($dir.Name) + "\" + $($item.Name) + "\" + $($content.Name))
                    }
                }
                else {
